...
class DetailTomatoesSpider(scrapy.Spider): 
    ...
    a2 = json.load(files2)
    list_id = [i["imdb_id"] for i in a2]
    print("list-id {}".format(len(list_id)))
    start_urls = [
        "https://www.rottentomatoes.com"
    ]
    def parse(self, response):
        for i in self.a2:
          try:
            if i["url"]!="":
              item = RottentomatoesItem()
              item["imdb_title"] = i["imdb_title"]
              item["imdb_id"] = i["imdb_id"]
              headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/45.0.2454.85 Safari/537.36'}
    ...
    def parse_dir_contents(self, response):
        ...
        try:
          ...
          item["tomatometer_score"] = tomatometerscore
          item["audience_score"] = audiencescore
          actors = []
          #get information
          item["tomatometer_number"] = response.xpath('//*[@id="topSection"]/div[1]/score-board/a[1]/text()').extract()[0]
          item["audience_number"] = response.xpath('//*[@id="topSection"]/div[1]/score-board/a[2]/text()').extract()[0]

          for li in response.css('li[class="meta-row clearfix"]'):
            label = li.xpath('div[1]/text()').extract()[0]
            if label =="Genre:":
                item["genre"] = li.xpath('div[2]/text()').extract()[0].replace("\n","")
            if label =="Original Language:":
                item["original_language"] = li.xpath('div[2]/text()').extract()[0].replace("\n","")
            if label =="Release Date (Theaters):":
                item["theater_release_date"] = li.xpath('div[2]/time/@datetime').extract()[0].replace("\n","")
            if label == "Release Date (Streaming):":
                item["streaming_release_date"] = li.xpath('div[2]/time/@datetime').extract()[0].replace("\n","")
            if label == "Runtime:":
                item["runtime"] = li.xpath('div[2]/time/text()').extract()[0].replace("\n","")
        #get actor
        
          for li in response.css('div[data-qa="cast-crew-item"]'):
            actors.append({
              "link":li.xpath('div[2]/a/@href').extract()[0],
              "name":li.xpath('div[2]/a/span/text()').extract()[0].replace("\n","")
            })
          item["cast"] = actors
        except:
          pass
